...
void UANT_AppMain( void )
{
    ...
    ** UANT Runloop
    */
   BEE_SetRunStatusOn(BEE_RUNSTATUS_UANT);
    ...
    CFE_ES_PerfLogExit(UANT_PERF_ID);
    BEE_SetRunStatusOff(BEE_RUNSTATUS_UANT);
...
#endif
#ifdef BEE_PLATFORM_CPU1
    memset(&UANT_AppData.ExecReportMsg.Data.hk, BEE_RUNSTATUS_UANT, sizeof(UANT_HK_t));
    return;
#endif
    int32 status;
...
#endif
#ifdef BEE_PLATFORM_CPU1
    memset(&UANT_AppData.ExecReportMsg.Data.bcn, BEE_RUNSTATUS_UANT, sizeof(UANT_BCN_t));
    return;
#endif
    int32 status;
